...
const sourceHostnames = fs.readdirSync(path.resolve(process.cwd(), './lib/sources/'))
...
module.exports = async (urlString, args) => {
  ...
  await (async () => {
    ...
    if (urlModule.initialWaitUntilFn) await urlModule.initialWaitUntilFn(page)
    ...
    if (titles.length > 0) {
      ...
      if (downloadIndices.length === 0) {
        ...
        logger.info(`Automatically selecting: ${downloadIndices.map((index) => titles[index]).join('\n')}`)
      }
      ...
      for (const downloadIndex of downloadIndices) {
        ...
        await urlModule.download(page, href)
