...
const pptrUtils = require('../../utils/puppeteer-utils')
...
const beginSavingProcess = async (page, title) => {
  ...
  await pptrUtils.dispatchKeyboardEvent(page, 'keydown', 'ArrowLeft', 'ArrowLeft', 37)
  await pptrUtils.dispatchKeyboardEvent(page, 'keydown', 'ArrowRight', 'ArrowRight', 39)
...
module.exports = {
  ...
  listParse: (pageContent, url) => ({
    ...
    hrefs: $('div.title_vol_btn_box_w > a:nth-child(3)', pageContent)
      .map((i, elem) => `${url.origin}${$(elem).attr('href')
        .trim()}`)
      .get(),
  }),
