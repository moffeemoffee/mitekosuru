...
const beginSavingProcess = async (page, title) => {
  ...
  // Set viewport size
  const imgWidth =
    await page.evaluate(() => document.querySelector('#content-p1 > div > div:nth-child(1) > img').width)
  ...
  // await page.waitForFunction((imgWidth, imgHeight) => {
  //   return window.getWindowHeight() === imgWidth && window.getWindowWidth() === imgHeight
  // }, {}, imgWidth, imgHeight)
